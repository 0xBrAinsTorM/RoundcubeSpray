...
parser.add_argument('--usernames', '-U', type=str, required=True, help='Single username or file containing usernames')
...
parser.add_argument('--sleep', '-s', type=float, default=0, help='Time to wait after each password round in seconds')
...
def try_login(url, username, password):
    ...
    try:
        ...
        response = session.post(login_page_url, data=login_data, headers=headers, allow_redirects=False, timeout=30)
...
def password_spraying(url, usernames, passwords, timeout, sleep_time):
    for password in passwords:
        ...
        # Sleep after a round of attempts with a single password
        if sleep_time > 0:
            print(f"Sleeping for {sleep_time} seconds after trying password '{password}' on all usernames.")
...
if __name__ == "__main__":
    ...
    password_spraying(args.url, usernames, passwords, args.timeout, args.sleep)
